(function() {
  var ProxyServer, _, crypto, elf, fs, h, http, insert_counter, pg, query_counter, query_error_counter, sys, update_counter, upsert_counter, upsert_error_counter, upsert_row_counter, url;
  sys = require('sys');
  fs = require('fs');
  http = require('http');
  url = require('url');
  ...
  ProxyServer.prototype.startServer = function(callback) {
    ...
    server = http.createServer(function(req, resp) {
      var action, credentials, dbName, parts, stats;
      ...
      if (url.parse(req.url).pathname === '/stats') {
        ...
        return resp.end(JSON.stringify(stats));
      } else {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        (function() {
          ...
          return _result.push((function() {
            ...
            return h.execSqlCount(client, query.table, row, function(err, rowCnt) {
              var res, sql;
              if (err) {
